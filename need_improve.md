...
2. attribution.py
  （改进 54 已覆盖幻觉 id 校验、跨 session 串号、fallback 污染）残留建议：run() 里 _apply 的事务段仍无 try/except，非幻觉类异常（如 DB 断连）仍会冒泡中断整个离线批处理，可加会话级 try/except 兜底；在线侧 graph.py `_select_dimensions` 的 DimensionSelection ids 同样无白名单校验。
