...
class DimensionDelta(BaseModel):
    name: str = Field(description="性格维度名称")
    delta_value: float = Field(default=0.0, ge=-0.5, le=0.5, description="维度值变化，范围 -0.5 到 0.5")
...
class AttributionEngine:
    ...
    def _pair_dialogues(self, logs: list[DialogueLog]) -> list[tuple[DialogueLog, DialogueLog]]:
        # Sort ascending by time so user message precedes assistant reply
...
1. 判断哪些性格维度促成了成功或失败。
2. 给出每个维度的 delta_value（-0.5 到 0.5）、delta_success、delta_failure 和原因。
3. 如果这条对话提炼出值得记住的人格事件（例如新的口头禅、态度转变、重要经验），请写 event_summary；否则留空。
"""
...
请按 JSON 格式返回：{{"deltas": [{{"name": "维度名", "delta_value": 0.1, "delta_success": 1, "delta_failure": 0, "reason": "...", "event_summary": "..."}}]}}
"""
        ...
        try:
            ...
            # Fallback empty result on parse failure
    ...
    def _apply(self, result: AttributionResult, dialogue_id: Optional[int]) -> None:
        for delta in result.deltas:
            ...
            # Check pruning
            dim = self.pg.get_dimension(delta.name)
            if dim and dim.failure_count >= settings.failure_threshold:
                self.pg.deactivate_dimension(delta.name)
